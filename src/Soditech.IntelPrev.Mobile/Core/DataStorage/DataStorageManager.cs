...
using Soditech.IntelPrev.Mobile.Core.Secure;
using Soditech.IntelPrev.Mobile.Core.Type;
...
public class DataStorageManager :  IDataStorageManager
{
    ...
    private static T RetrieveObject<T>(string key, T defaultValue = default(T))
    {
    ...
    public T Retrieve<T>(string key, T defaultValue = default(T), bool shouldDecrpyt = false)
    {
        var value = TypeHelperExtended.IsPrimitive(typeof(T), false) ?
            ...
            DataStorageManager.RetrieveObject(key, defaultValue);
