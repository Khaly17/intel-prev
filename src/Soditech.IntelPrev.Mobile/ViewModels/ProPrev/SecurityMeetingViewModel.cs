...
class SecurityMeetingViewModel : MauiViewModel
{
    ...
    {
        var result = await _proxyClientService.GetAsync<PreventionContentResult>(PreventionRoutes.ProPrevSettings.GetSecurityQuarterContent);
        if (result.IsSuccess)
        {
