...
class TrainingViewModel: MauiViewModel
{
    ...
    {
        var result = await _proxyClientService.GetAsync<PreventionContentResult>(PreventionRoutes.ProPrevSettings.GetHealthFormationContent);
        if (result.IsSuccess)
        {
