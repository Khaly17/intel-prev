...
class FirstAidViewModel : MauiViewModel
{
    ...
    {
        var result = await _proxyClientService.GetAsync<PreventionContentResult>(PreventionRoutes.ProPrevSettings.GetFirstAidKitContent);
        if (result.IsSuccess)
        {
