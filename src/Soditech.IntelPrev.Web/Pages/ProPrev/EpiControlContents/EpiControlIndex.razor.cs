...
namespace Soditech.IntelPrev.Web.Pages.ProPrev.EpiControlContents;

public partial class EpiControlIndex
{
    [Inject]
    private ILogger<EpiControlIndex> Logger { get; set; } = default!;
    ...
    private async Task SaveContent()
    {
        ...
        try
        {
            ...
            {
                var result = await ProxyService.PostAsync<ProPrevContentResult>(PreventionRoutes.ProPrevSettings.UpdateEpiControlContent, NewProPrevContentResult);
    ...
    {
        var result = await ProxyService.GetAsync<ProPrevContentResult>(PreventionRoutes.ProPrevSettings.GetEpiControlContent);
        if (result.IsSuccess)
        {
