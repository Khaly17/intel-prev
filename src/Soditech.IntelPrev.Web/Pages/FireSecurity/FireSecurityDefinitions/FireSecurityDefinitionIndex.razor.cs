...
namespace Soditech.IntelPrev.Web.Pages.FireSecurity.FireSecurityDefinitions;

public partial class FireSecurityDefinitionIndex
{
    [Inject]
    private ILogger<FireSecurityDefinitionIndex> Logger { get; set; } = default!;
    ...
    private async Task SaveContentAsync()
    {
        ...
        try
        {
            ...
            {
                var result = await ProxyService.PostAsync<FireSecuritySettingContentResult>(PreventionRoutes.FireSecuritySettings.UpdateDefinitionContent, NewFireSecuritySettingContentResult);

                HandleResult(result, "La definition a été ajouté avec succès !");
            }
            else
            {
                var result = await ProxyService.PostAsync<FireSecuritySettingContentResult>(PreventionRoutes.FireSecuritySettings.UpdateDefinitionContent, NewFireSecuritySettingContentResult);
