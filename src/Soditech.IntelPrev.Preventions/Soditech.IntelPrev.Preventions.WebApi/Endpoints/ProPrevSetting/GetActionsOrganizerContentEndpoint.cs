...
[HttpGet(PreventionRoutes.ProPrevSettings.GetActionsOrganizerContent)]
[Tags("ActionsOrganizer")]
public class GetActionsOrganizerContentEndpoint(IServiceProvider serviceProvider): EndpointWithoutRequest<TResult<ProPrevContentResult>>
{
    private readonly IMediator _mediator = serviceProvider.GetRequiredService<IMediator>();
    
    public override async Task<TResult<ProPrevContentResult>> HandleAsync(CancellationToken cancellationToken)
    {
        var request = new ActionsOrganizerContentQuery();
