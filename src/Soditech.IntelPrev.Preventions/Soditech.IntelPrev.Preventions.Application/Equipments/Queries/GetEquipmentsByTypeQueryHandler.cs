...
public class GetEquipmentsByTypeQueryHandler(IServiceProvider serviceProvider) : IRequestHandler<GetEquipmentsByTypeQuery, TResult<IEnumerable<EquipmentResult>>>
{
    ...
    public async Task<TResult<IEnumerable<EquipmentResult>>> Handle(GetEquipmentsByTypeQuery request, CancellationToken cancellationToken)
    {
        try
        {
            var isTypeValid = Enum.TryParse<EquipmentType>(request.Type,  out var type);

            if (!isTypeValid)
            {
                return Result.Failure<IEnumerable<EquipmentResult>>(new Error("400", "Invalid equipment type"));
            }
