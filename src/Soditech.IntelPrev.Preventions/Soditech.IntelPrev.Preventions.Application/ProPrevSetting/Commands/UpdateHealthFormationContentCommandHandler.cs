...
public class UpdateHealthFormationContentCommandHandler(IServiceProvider serviceProvider) : IRequestHandler<HealthFormationContentCommand, Result>
{
    ...
    private readonly ILogger<UpdateHealthFormationContentCommandHandler> _logger =
        serviceProvider.GetRequiredService<ILogger<UpdateHealthFormationContentCommandHandler>>();

    /// <inheritdoc />
    public async Task<Result> Handle(HealthFormationContentCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            proPrevSettings.HealthFormationContent = request.Content;
        ...
        {
            _logger.LogError(ex, "Error while updating Health Formation Content");
            return Result.Failure(new Error("500","Error while updating Health Formation Content"));
        }
    }
}