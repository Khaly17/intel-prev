...
public class UpdateFireConsignsContentCommandHandler(IServiceProvider serviceProvider) : IRequestHandler<UpdateFireConsignsContentCommand, Result>
{
    ...
    private readonly ILogger<UpdateFireConsignsContentCommandHandler> _logger =
        serviceProvider.GetRequiredService<ILogger<UpdateFireConsignsContentCommandHandler>>();

    /// <inheritdoc />
    public async Task<Result> Handle(UpdateFireConsignsContentCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            proPrevSettings.FireConsignsContent = request.Content;
        ...
        {
            _logger.LogError(ex, "Error while updating Cse Agenda Content");
            return Result.Failure(new Error("500","Error while updating Cse Agenda Content"));
        }
    }
}