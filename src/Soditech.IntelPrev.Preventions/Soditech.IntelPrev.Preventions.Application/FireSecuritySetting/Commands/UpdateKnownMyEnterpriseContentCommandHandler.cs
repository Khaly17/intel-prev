...
public class UpdateKnownMyEnterpriseContentCommandHandler(IServiceProvider serviceProvider) : IRequestHandler<UpdateKnownMyEnterpriseContentCommand, Result>
{
    ...
    private readonly ILogger<UpdateKnownMyEnterpriseContentCommandHandler> _logger =
        serviceProvider.GetRequiredService<ILogger<UpdateKnownMyEnterpriseContentCommandHandler>>();

    /// <inheritdoc />
    public async Task<Result> Handle(UpdateKnownMyEnterpriseContentCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            proPrevSettings.KnownMyEnterpriseContent = request.Content;
